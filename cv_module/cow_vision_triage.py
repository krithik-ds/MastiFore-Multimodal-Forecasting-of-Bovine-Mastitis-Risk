...
class CowVisionTriageEngine:
    ...
    def analyze_frame(self, frame):
        ...
        cv2.putText(frame, "MastiFore - TIER 1 BARN CCTV TELEMETRY MESH",
    ...
    def _detect_cows(self, frame):
        ...
        # Contour Fallback (Robust Quadruped Bovine Filter)
        ...
        for i, cnt in enumerate(contours):
            ...
            if area > min_area:
                ...
                # Must be a horizontal quadruped body (rejects vertical humans < 1.05 and short stray dogs < min_height)
                if 1.05 <= aspect <= 3.2 and h >= min_height:
                    detections.append({
                        'id': i + 1,
                        'bbox': [x, y, x + w, y + h],
                        'conf': 0.85
    ...
    def _track_differential_chews(self, cow_id, roi, cow_w, cow_h, facing):
        ...
        if num_recent_chews >= 2:
            ...
            raw_cpm = round((num_recent_chews / time_span) * 60.0, 1)
            # Biological bovine rumination window: 20 - 85 CPM (rejects erratic rapid panting > 85 CPM)
            if 20.0 <= raw_cpm <= 85.0:
                cpm = raw_cpm
                is_eating = True
